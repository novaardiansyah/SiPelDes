...
namespace App\Jobs\RelocationLetterResource;
...
class GeneratePdfJob implements ShouldQueue
{
  ...
  public function __construct(
    public RelocationLetter $record,
    public Model $user
  ) { 
  ...
  public function handle(): void
  {
    ...
    $mpdf = new \Mpdf\Mpdf();
    $mpdf->WriteHTML(view('pdf.RelocationLetterResource.letter', compact('data', 'record')));
    $mpdf->Output(storage_path("app/public/{$filepath}"), 'F');

    $download = explode('/', $filepath)[array_key_last(explode('/', $filepath))];

    Notification::make()
      ->title('Surat Keterangan Pindah ('.$kode_surat.')')
